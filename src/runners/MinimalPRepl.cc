...
int main(int argc, char *argv[]){
  if(argc < 10){
    printf("\nUSAGE: ./MinimalPSelection <path to inits> "
           ...
           "k: antibody-mediated neutralization rate\n\n"
           "t_M: time of antibody response\n\n"
  ...
  double t_M = atof(argv[7]);
  string out_path = argv[8];
  bool threshold = atoi(argv[9]);
  ...
  int transformed_seed = int(params.get_integer_parameter("rng_seed") + bn * 8 * (k + 1) * (t_M + 1));
  ...
  params.set_parameter("K", k);
  params.set_parameter("T_M", t_M);
